...
Use this diff-friendly format to exercise one certificate install against a dedicated CSSL profile on a BIG-IP.
Everything below is copy/paste-ready for editors like Notepad++.
...
Variables for device, profile, and PEM sources. Collect all device details and PEM paths in one place.
...
Final confirmation before install. Double-check the device and profile before performing the single install.
