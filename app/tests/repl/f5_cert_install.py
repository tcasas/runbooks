...
def configure_logger(level: str = "INFO") -> logging.Logger:
    """Configure a module-level logger for REPL and CLI runs."""

    log_level = getattr(logging, level.upper(), logging.INFO)
    logging.basicConfig(level=log_level, format="%(asctime)s %(levelname)s %(message)s")
    return logging.getLogger("repl-f5cert")


def run_install(perform_install: bool, *, logger: Optional[logging.Logger] = None):
    """Run readiness checks and optionally trigger the install."""

    logger = logger or configure_logger()
    device = lookup_device()
    f5cert = build_cert_attachment(device)
    f5cert.logger = logger

    log_readiness(f5cert)

    if not perform_install:
        logger.info("Dry run only. Pass --install to execute the certificate push.")
        return

    result = f5cert.install()
    logger.info("Install result: %s", result)


def parse_args():
    parser = argparse.ArgumentParser(description="Exercise a single F5Cert install.")
    parser.add_argument(
        "--install",
        action="store_true",
        help="Perform the install after readiness checks (defaults to dry run)",
    )
    parser.add_argument(
        "--log-level",
        default="INFO",
        help="Logging level for the helper (default: INFO)",
    )
...
if __name__ == "__main__":
    ...
    run_install(perform_install=args.install, logger=logger)
